...
export const addPost = (req, res) => {
  // res.json("this is post route from controller");
  ...
  jwt.verify(token, "jwtkey", (err, userInfo) => {
    ...
    const q = "INSERT INTO post(title, des, img, cat, date, uid) VALUES(?)";
    const values = [
      req.body.title,
      req.body.des,
      req.body.img,
...
export const updatePost = (req, res) => {
  ...
  jwt.verify(token, "jwtkey", (err, userInfo) => {
    ...
    const q = "UPDATE post SET tit=?, des=?, img=?, cat=? WHERE id=? AND uid=?";
